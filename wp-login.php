...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	if ( $wp_error->has_errors() ) {
		...
		if ( ! empty( $messages ) ) {
			 ...
			 */
			echo '<p class="message">' . 'Регистрация завершена.На вашу почту вам придёт письмо с активацией пароля .<a href="https://volonteria.vercel.app/">Теперь вы можете войти на сайт</a>' . "</p>\n";
		}
	}
} // End of login_header().
...
switch ( $action ) {
	...
	case 'retrievepassword':
		...
		<p id="nav">
			<a href="https://volonteria.vercel.app/"><?php _e( 'Log in' ); ?></a>
			<?php
	...
	case 'rp':
		...
		if ( ( ! $errors->has_errors() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
			...
			setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
			login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="https://volonteria.vercel.app/">' . __( 'Log in' ) . '</a></p>' );
			login_footer();
			exit;
		}
	...
	case 'register':
		...
		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
			...
			<p id="access_reg">
				<?php _e( 'Нажимая кнопку регистрация, вы соглашаетесь с условиями ' ); ?><a href="https://volonteria.vercel.app/Rules">оферты</a>
			</p>
			<br class="clear" />
			<input type="hidden" name="redirect_to" value="<?php echo esc_attr( $redirect_to ); ?>" />
			<p class="submit">
